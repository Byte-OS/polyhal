use arch::{addr::PhysPage, PAGE_SIZE};
use buddy_system_allocator::LockedFrameAllocator;
use spin::Lazy;

static LOCK_FRAME_ALLOCATOR: Lazy<LockedFrameAllocator<32>> = Lazy::new(|| LockedFrameAllocator::new());
...
pub fn frame_alloc() -> PhysPage {
    let ppn = LOCK_FRAME_ALLOCATOR.lock().alloc(1).expect("can't find memory page");
    PhysPage::new(ppn)
}
