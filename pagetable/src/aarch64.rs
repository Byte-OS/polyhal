...
use polyhal_base::{addr::{PhysAddr, PhysPage, VirtAddr, VirtPage}, utils::bit};
...
impl PTE {
    ...
    pub(crate) fn new_table(ppn: PhysPage) -> Self {
        Self(ppn.to_paddr().raw() | 0b11)
    }

    #[inline]
    pub(crate) fn new_page(ppn: PhysPage, flags: PTEFlags) -> Self {
        Self(ppn.to_paddr().raw() | flags.bits() as usize)
    }
}
...
#[inline]
pub fn pn_index(vpage: VirtPage, n: usize) -> usize {
    (vpage.raw() >> 9 * n) & 0x1ff
}
