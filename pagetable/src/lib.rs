...
use polyhal_base::{
    ...
    config::PAGE_SIZE, utils::bit,
};

pub(self) const PTE_NUM_IN_PAGE: usize = PAGE_SIZE / size_of::<PTE>();
...
impl PageTable {
    ...
    pub(crate) fn get_pte_list(paddr: PhysAddr) -> &'static mut [PTE] {
        paddr.to_vaddr().slice_mut_with_len::<PTE>(PTE_NUM_IN_PAGE)
    }
    ...
    pub fn translate(&self, vaddr: VirtAddr) -> Option<(PhysAddr, MappingFlags)> {
        ...
        Some((
            PhysAddr::new(pte.address().0 + pn_offest(vaddr, 0)),
            pte.flags().into(),
        ))
    }
